...
export default function Button({ children, variant = 'edit', onClick, icon }: ButtonProps) {
  const variants = {
  ...
  return (
    <button
      onClick={onClick}
      className={`inline-flex items-center justify-center gap-1.5 text-xs font-medium ${isIconOnly ? 'px-2' : 'px-3'} py-1.5 rounded-md cursor-pointer transition-opacity font-[Inter] ${variants[variant]}`}
    >
